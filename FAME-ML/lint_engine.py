...
# - Replaced all print() statements with logging.warning() for security events
# - Added specific context (function/class names) to logs
...
# Configure logging
logging.basicConfig(filename='forensics.log', level=logging.INFO, format='%(asctime)s %(name)s [%(levelname)s] (%(process)d) %(message)s', datefmt='%Y-%m-%d %H:%M:%S %z')
...
def getDataLoadCount( py_file ):
    ...
    for def_ in func_def_list:
        ...
        if(( class_name == constants.TORCH_KW ) and (func_name == constants.LOAD_KW ) ):
            data_load_count += 1 
            logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_DATA_LOAD} detected [{class_name}.{func_name}] at line {func_line} in {py_file}")
...
def getDataLoadCountb( py_file ):
    ...
    for assign_ in func_assign_list:
        ...
        if( (func_name == constants.GET_LOADER_KW ) and (len(func_arg_list) > 0) ):
            data_load_countb += 1 
            logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_DATA_LOAD} detected [{func_name}] at line {func_line} in {py_file}")
...
def getModelLoadCounta( py_file ):
    ...
    for def_ in func_def_list:
        ...
        if(( class_name == constants.DEEP_SPEECH_KW ) and (func_name == constants.LOAD_MODEL_PACKAGE_KW) ):
            model_load_counta += 1 
            logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_MODEL_LOAD} detected [{class_name}.{func_name}] at line {func_line} in {py_file}")
...
def getModelLoadCountb( py_file ):
    ...
    for assign_ in func_assign_list:
        ...
        if( (func_name == constants.PATCH_PATH_KW  ) and (len(func_arg_list) > 0) ):
            model_load_countb += 1 
            logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_MODEL_LOAD} detected [{func_name}] at line {func_line} in {py_file}")
...
def getDataDownLoadCount( py_file ):
    ...
    for def_ in func_def_list:
        ...
        if(( class_name == constants.WGET_KW ) and (func_name == constants.DOWNLOAD_KW ) ):
            ...
            logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_DATA_DLOAD} detected [{class_name}.{func_name}] at line {func_line} in {py_file}")
...
def getDataDownLoadCountb( py_file ):
    ...
    for func_ in func_assign_list:
        ...
        if( (func_name == constants.PREPARE_URL_IMAGE_KW ) and (len(func_arg_list) > 0) ):
            ...
            logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_DATA_DLOAD} detected [{func_name}] at line {func_line} in {py_file}")
...
def getModelFeatureCount( py_file ):
    ...
    for feature_ in feature_list:
        ...
        if( (class_name == constants.DATA_KW ) and (feature_name == constants.HP_BATCH_SIZE_KW ) ):
            ...
            logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_MODEL_FEATURE} detected [{class_name}.{feature_name}] at line {feature_line} in {py_file}")
        ...
        for var_name in lhs:
            if ( constants.LABEL_KW in var_name):
                if( (func_name == constants.READ_H5FILE_KW  ) and (len(func_arg_list) > 0) ):
                    model_label_count += 1 
                    logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_MODEL_LABEL} detected [{func_name}] at line {func_line} in {py_file}")
    ...
    for assign_ in func_assign_list:
        ...
        if ( constants.LABEL_KW in lhs):
        	...
        	if ( (var_s == constants.SENT_KW ) and (var_d == constants.SENT_KW )  and (rhs_var_iter == constants.INPUT_BATCH_LIST_KW ) ):
        		...
        		logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_MODEL_LABEL} detected [{lhs}] at line {func_line} in {py_file}")
...
def getModelOutputCount( py_file ):
    ...
    for def_ in func_def_list:
        ...
        if(( class_name == constants.DATA_KW ) and (func_name == constants.SHOW_DATA_SUMMARY_KW ) ):
            ...
            logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_MODEL_OUTPUT} detected [{class_name}.{func_name}] at line {func_line} in {py_file}")
...
def getModelOutputCountb( py_file ):
    ...
    for assign_ in func_assign_list:
        ...
        if( (func_name == constants.GET_TENSOR_KW ) and (len(func_arg_list) > 0) ):
            ...
            logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_MODEL_OUTPUT} detected [{func_name}] at line {func_line} in {py_file}")
...
def getDataPipelineCount( py_file ):
    ...
    for def_ in func_def_list:
        ...
        if(( class_name == constants.ARG_PARSE_KW ) and (func_name == constants.ARGUMENT_PARSER_KW ) and (len(arg_call_list) > 0)):
            ...
            logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_PIPELINE} detected [{class_name}.{func_name}] at line {func_line} in {py_file}")
...
def getDataPipelineCountb( py_file ):
    ...
    for assign_ in func_assign_list:
        ...
        if( (func_name == constants.TRAIN_EVAL_PIPELINE_CONFIG_KW ) ):
            ...
            logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_PIPELINE} detected [{func_name}] at line {func_line} in {py_file}")
...
def getDataPipelineCountd( py_file ):
	...
	for feature_ in feature_list:
		...
		if( (class_name == constants.PIPELINE_CONFIG_KW  ) and (feature_name == constants.MODEL_KW ) ):
			...
			logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_PIPELINE} detected [{class_name}.{feature_name}] at line {feature_line} in {py_file}")
...
def getEnvironmentCount( py_file ):
    ...
    for def_ in func_def_list:
        ...
        if(( class_name == constants.WRAPPED_ENV_KW ) and (func_name == constants.STEP_KW ) and (len(arg_call_list) > 0)):
            environment_count += 1 
            logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_REL_ENV} detected [{class_name}.{func_name}] at line {func_line} in {py_file}")
...
def getEnvironmentCountb( py_file ):
	...
	for feature_ in feature_list:
		...
		if( (class_name == constants.OBSERVATION_SPACE_KW  ) and (feature_name == constants.SHAPE_KW ) ):
			...
			logging.warning(f"SECURITY_EVENT: {constants.CONSOLE_STR_REL_ENV} detected [{class_name}.{feature_name}] at line {feature_line} in {py_file}")
